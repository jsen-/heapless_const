...
impl Atomic {
    ...
    fn load_acquire(&self) -> usize {
        unsafe { usize::load_acquire(self.v.get()) }
    }

    fn load_relaxed(&self) -> usize {
        usize::load_relaxed(self.v.get())
    }

    fn store_release(&self, val: usize) {
        unsafe { usize::store_release(self.v.get(), val) }
    }
}
...
impl<T, const N: usize> Queue<T, N> {
    const X: MaybeUninit<T> = MaybeUninit::uninit();
...
mod tests {
    ...
    fn u8() {
        let mut rb: Queue<u8, 256, _> = Queue::u8();
